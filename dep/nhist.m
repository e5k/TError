...
%     figure('Name', Title);
    Hx = axes('Box', 'on', 'FontSize', AxisFontSize);
...
%           doesn't allow changing the lineColor property of a histc bar graph.)
            if vertLinesFlag % then plot the bars with edges
                bar(x{k}+binWidth(k)/2,n{k}/1,'FaceColor',faceStyleOrder{k},'barwidth',1,'EdgeColor','k','linewidth',0.3)
...
%   Plot the boxplot!
    if boxplotFlag % it is important to set the ylim property here so errorb plots the appropriate sized error bars
        ...
        if normalHist % plot the thing with all the colors, there is just one of them.
            bplot(cellValues{k},tempY,'barwidth',boxplotWidth,'linewidth',linewidth,'horizontal',pointsOrNo,'specialwidth','histmode', 'color', 'k');
...
if horizontalFlag
    hReg2(end+1) = rectangle('Position',[boxEdge(1),y-barWidth/2,IQR,barWidth],'linewidth',linewidth,'EdgeColor','k', 'FaceColor', 'w');
    ...
    hReg2(end+1) = rectangle('Position',[y-barWidth/2,boxEdge(1),barWidth,IQR],'linewidth',linewidth,'EdgeColor',boxColor, 'FaceColor', 'r');
    hold on;
...
if outlierFlag % but only if you want to
    ...
    if ~isempty(yy)
        ...
        if ~isempty(xx)
            if horizontalFlag
                hReg2(6) = plot(xx,yy,'.','linewidth',linewidth,'color',[.5 .5 .5], 'MarkerSize', 7);
            else
